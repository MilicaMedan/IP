...
public class HelpCallDAO {
	...
	private static final String SQL_DELETE="DELETE FROM helpcall WHERE id=?";
	private static final String SQL_REPORT="UPDATE helpcall SET falseNews=1 WHERE id=?";
	...
	public static void report(int id){
		...
		try {
			...
			PreparedStatement stmt = conn.prepareStatement(SQL_REPORT);
			stmt.setInt(1, id);
			stmt.executeUpdate();
			stmt.close();
		} catch (Exception ex) {
